...
import (
	...
	"github.com/ijsnow/makaisailing/server/db"
	"github.com/ijsnow/makaisailing/server/email"
...
func NewApp(opts ...AppOptions) *App {
	...
	app.Engine.Use(func(h echo.HandlerFunc) echo.HandlerFunc {
		return func(c *echo.Context) error {
			...
			if err != nil && err.Error() == http.StatusText(http.StatusNotFound) {
				...
				// omit first `/`
				if _, errr := Asset(c.Request().URL.Path[1:]); errr == nil {
	...
	emailUser, _ := app.Conf.String(prefix + ".email.user")
	emailPass, _ := app.Conf.String(prefix + ".email.password")

	app.Emailer = email.NewEmailer(emailUser, emailPass)
